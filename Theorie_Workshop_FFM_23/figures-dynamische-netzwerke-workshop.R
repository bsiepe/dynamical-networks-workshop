...
wi_mat <- matrix(c(0,.9,0,0), nrow = 2)
bw_mat <- matrix(c(0,0,-.9,0), nrow = 2)

svg("typingspeed.svg")
par(mfrow = c(1,2))
qgraph(bw_mat, theme = "colorblind", layout = "spring", labels = node_names,
       vsize = 18, esize = 8, directed = FALSE, title = "Between", title.cex = 1.7, negDashed = TRUE)

qgraph(wi_mat, theme = "colorblind", layout = "spring", labels = node_names,
       vsize = 18, esize = 8, directed = FALSE, title = "Within", title.cex = 1.7)
dev.off()

...
-   ggf. standardisieren/durch Konstante teilen
    -   geht auch innerhalb von GIMME (`standardize = TRUE`)
...
Mittelwerte und Standardabweichungen von Pfadkoeffizienten betrachten (vgl. Wright et al., 2022): - aber: verzerrt dadurch, dass viele Edges auf 0 gesetzt werden!
...
indiv_path_estimates |> 
  mutate(across(c(beta, se, z, pval),
                ~round(., digits = 3))) |> 
  head(n = 6L) |> 
  gt() 
...
-   Check: Residuen auf zeitliche Korreliertheit überprüfen (Beltz & Gates, 2016)
...
-   Algorithmus resultiert oftmals in mehreren äquivalente Lösungen
...
-   kann keine Subgruppen schätzen!
...
## Interessante Anwendungen {.smaller}
...
-   Vorhersage von Therapieverläufen mit Netzwerkparametern:
    -   Hehlmann et al. (submitted, s. Kursmaterialien)
    -   Lutz et al. (2018)
-   Zentralität (Beck & Johnson, 2022)
-   Klinische Fallstudien (z.B. Ong et al., 2022)
-   Feedback mit EMA (z.B. v. Klipstein et al. (2023), Rimpler et al. (in prep., s. Kursmaterialien))
